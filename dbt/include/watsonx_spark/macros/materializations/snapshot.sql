{% macro watsonx_spark__snapshot_hash_arguments(args) -%}
    md5({%- for arg in args -%}
...
{% macro watsonx_spark__snapshot_string_as_time(timestamp) -%}
...
{% macro watsonx_spark__post_snapshot(staging_relation) %}
    {% do adapter.drop_relation(staging_relation) %}
{% endmacro %}


{% macro watsonx_spark__create_columns(relation, columns) %}
    {% if columns|length > 0 %}
    {% call statement() %}
...
{% materialization snapshot, adapter='watsonx_spark' %}
