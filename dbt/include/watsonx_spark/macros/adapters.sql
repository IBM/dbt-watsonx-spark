...
{% macro watsonx_spark__tblproperties_clause() -%}
...
{% macro watsonx_spark__options_clause() -%}
...
{% macro watsonx_spark__comment_clause() %}
...
{% macro watsonx_spark__partition_cols(label, required=false) %}
...
{% macro watsonx_spark__clustered_cols(label, required=false) %}
...
{% macro watsonx_spark__persist_constraints(relation, model) %}
  {%- set contract_config = config.get('contract') -%}
...
{% macro watsonx_spark__alter_table_add_constraints(relation, constraints) %}
...
{% macro watsonx_spark__alter_column_set_constraints(relation, column_dict) %}
...
{% macro watsonx_spark__rename_relation(from_relation, to_relation) -%}
...
{% macro watsonx_spark__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro watsonx_spark__persist_docs(relation, model, for_relation, for_columns) -%}
  {% if for_columns and config.persist_column_docs() and model.columns %}
    {% do alter_column_comment(relation, model.columns) %}
  {% endif %}
{% endmacro %}

{% macro watsonx_spark__alter_column_comment(relation, column_dict) %}
...
{% macro watsonx_spark__make_temp_relation(base_relation, suffix) %}
...
{% macro watsonx_spark__alter_column_type(relation, column_name, new_column_type) -%}
...
{% macro watsonx_spark__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
