from dbt.adapters.spark.http_auth.authenticator import Authenticator
...
class WatsonxData(Authenticator):
    ...
    def _get_cpd_token(self, cpd_env):
        ...
        token = Token(response.get("token"),
                      self._extract_expiry(response.get("token")))
        return token
    ...
    def _get_sass_token(self, sass_env):
        ...
        token = Token(response.get("accessToken"),
                      self._extract_expiry(response.get("accessToken")))
        return token
    ...
    def _extract_expiry(self, token):
        expire = jwt.decode(str(token), options={"verify_signature": False})
        return expire["exp"]
